...
try:
    ...
    ### NOTE: Hard coded libmpv file as the find_library doesn't find libmpv when installed through homebrew
    if sofile is None:
        sofile = "/opt/homebrew/lib/libmpv.2.dylib"
    backend = CDLL(sofile)
except:
    raise OSError("An error occurred loading libmpv. If you have libmpv around but this script can't find it, consult the documentation for ctypes.util.find_library which this script uses to look up the library filename.")
